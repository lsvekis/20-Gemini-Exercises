...
function sheets_todosFromNotes() {
  const sheet = getActiveSheetOrError_();
  const notes = (sheet.getRange('A1').getValue() || '').toString().trim();
...
function sheets_emailDraftFromInputs() {
  ...
  const recipient = (sheet.getRange('B1').getValue() || '').toString().trim();
  const context = (sheet.getRange('C1').getValue() || '').toString().trim();
...
function sheets_tutorAnswerFromQuestion() {
  const sheet = getActiveSheetOrError_();
  const question = (sheet.getRange('A1').getValue() || '').toString().trim();
...
/**
 * EXAMPLE 9 — Categorize text in A2:A → category in B2:B
 */
...
/**
 * EXAMPLE 10 — Summarize notes in A2:A → B2:B
 */
...
/**
 * EXAMPLE 11 — Extract keywords/tags from A2:A → B2:B
 */
...
/**
 * EXAMPLE 12 — Suggest column headers from topic in A1 → row 2
 */
...
/**
 * EXAMPLE 13 — Generate formula from natural language in A1 → B1
 */
